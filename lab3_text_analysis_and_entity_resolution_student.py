...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda (k, v): len(v)).reduce(add)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    #return vendorRDD.map(lambda (k, v): (k, v)).takeOrdered(1, lambda (k, v): -len(v))
...
def tf(tokens):
    ...
    Returns:
        dictionary: af dictionary of tokens to its TF values
    """
    d = defaultdict(int)
    ...
    for token in tokens:
        d[token] += 1
    return dict(zip(d.keys(), np.array(d.values())/count))
...
# * ####~~For each of the unique tokens, count how many times it appears in the FUCKING CORPUS and then compute the IDF for that token: *N/n(t)*~~
...
def tfidf(tokens, idfs):
    ...
    tfIdfDict = { token: idfs[token] * tfs[token] for token in tfs.keys() }
    return tfIdfDict
...
avgSimDups = trueDupsRDD.map(lambda (k, v): v[0]).reduce(add)/trueDupsCount

nonDupsRDD = (sims.leftOuterJoin(goldStandard)).filter(lambda (k, v): v[1] is None)
nonDupsCount = nonDupsRDD.count()
avgSimNon = nonDupsRDD.map(lambda (k, v): v[0]).reduce(add)/nonDupsCount
