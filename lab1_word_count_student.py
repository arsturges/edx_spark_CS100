...
# TODO: Replace <FILL IN> with appropriate code
pluralLambdaRDD = wordsRDD.map(lambda s: s + 's')
...
wordCounts = wordPairs.reduceByKey(lambda k, v: sum([k, v]))
...
wordCountsCollected = (wordsRDD
                       .map(lambda w: (w, 1))
                       .reduceByKey(lambda k, v: sum([k, v]))
                       .collect())
...
wordCount(shakeWordsRDD).takeOrdered(15, lambda (k, v): -v)
