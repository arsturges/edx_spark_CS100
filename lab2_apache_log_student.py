...
dayHostCount = dayGroupedHosts.map(lambda (k, v): (k, len(v)))

dailyHosts = (dayHostCount
             .sortByKey()).cache()
#dayHostCount.takeOrdered(30)            
...
avgDailyReqPerHost = (sortedByDay
                      .map(
                        lambda (k, v): (k, (len(list(v))/len(set(v)))))
                      ).cache()
...
badRecords.map(lambda log: (log.endpoint, 1)).take(5)
badEndpointsCountPairTuple = badRecords.map(lambda log: (log.endpoint, 1))
badEndpointsCountPairTuple.reduceByKey(lambda a, b: a+b).takeOrdered(20, lambda (k, v): -v)
