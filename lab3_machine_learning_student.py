...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    x = IDandRatingsTuple
    return (x[0], (len(x[1]), (sum(x[1]))/float(len(x[1]))))
    #return (x[0], (len(x[1]), np.mean(x[1])))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda (k, v): (k, math.pow(v, 2)))

    # Compute the total squared error - do not use collect()
    totalError = squaredErrorsRDD.map(lambda (k,v) : v).sum()
...
#myUnratedMoviesRDD = (moviesRDD.<FILL IN>)
